...
// Company name in the header. Set BRAND in the environment, or leave it.
const BRAND = process.env.BRAND || "FINANCE";

const html = `<title>集团财务分析</title>
<style>
:root{
...
function cfMatrix(list, months, dir, title, hint) {
  ...
  var shown = rows.slice(0, 15), rest = rows.slice(15);
  ...
  if (rest.length) {
    body += '<tr><td class="first"><span class="nm">其他 ' + rest.length + ' 个科目</span></td>' +
      months.map(function (m) {
...
function viewCash(list, months) {
  ...
  return vhead("现金流", "以银行分录的对方科目判定钱的来源与去向。" +
    ...
    cfMatrix(list, months, "in", "钱从哪里进来", "以对方科目分组，逐月列出。只显示前 15 大。") +
    cfMatrix(list, months, "out", "钱花到哪里去", "以对方科目分组，逐月列出。只显示前 15 大。") +
...
function viewProduct(list, months) {
  var blocks = list.map(function (c) {
    ...
    return '<section class="card"><div class="lhead"><div><h3>' + esc(c.name) + '</h3>' +
      ...
      }).join("") + '<tr class="sub"><td class="first">营业合计</td>' +
