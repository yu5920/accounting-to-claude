...
| `TX` | Taxation | 不进营业损益 |
| `CL` | 开在销售发票行上的负债科目（`2xxx` 某代收代缴基金科目、`TAX-xxxx` 某税务控制科目） | **不是营收**，但会造成「开票总额 ≠ 营收」 |
...
```
JV-YYMM31-0006  3/31  Dr 2xxx（预收）128,400.00 / Cr 41xx,42xx,44xx（收入）   ← 跨月课程收入拉进 3 月
...
正常，不是错。差额是开在负债科目上的发票行（代收代缴基金、销售税）。
...
`5xxx` 场地费科目（236,800，AccType `CO`），把课程场地费算成**固定成本**。
...
一个想抓「员工培训」的关键字，会连「培训场地」这类科目一起命中 —— 而后者是随开班数变动的交付成本，不是固定的营运费用。**加规则前先算一次会改到哪些科目**，
不要只看想抓的那几个：
